    ...
    switch (choice) {
        ...
        case 13:
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (i <= n / 2) {
                    starsToPrint = 2 * i + 1;
                    spaceToPrint = n - 1 - 2 * i;
                } else {
                    int reversedIndex = (n - 1) - i;
                    starsToPrint = 2 * reversedIndex + 1;
                    spaceToPrint = n - 1 - 2 * reversedIndex;
                }
                for (int k = 0; k < spaceToPrint; k++) {
                    cout << " ";
                }
        ...
        case 14:
            ...
            for (int i = -n / 2; i <= n / 2; i++) {
                int dist = abs(i);
                int starsToPrint = n - 2 * dist;
                int spacesToPrint = 2 * dist;
                for (int k = 0; k < spacesToPrint; k++) cout << " ";
                for (int j = 0; j < starsToPrint; j++) cout << "*";
                cout << endl;
            }
            break;
        case 15:
            cin >> n;
            for (int i = 0; i < n; i++) {
                int starsToPrint = n - 2 * abs(n / 2 - i);
                int spacesToPrint = 2 * abs(n / 2 - i);
                for (int k = 0; k < spacesToPrint; k++) cout << " ";
                for (int j = 0; j < starsToPrint; j++) cout << "*";
                cout << endl;
            }
